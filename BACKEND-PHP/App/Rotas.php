        ...
        if (strlen($pin) == 8) {
            if ($auth['pin_gerado'] == $pin && $auth['senha'] == $senha) {
                ...
                if ($auth['pin_gerado'] != $pin) {
                    ...
                    error_log('[{"status":false,"msg":"PIN inválido","pin":"","senha":""}]');
                    if ($auth['senha'] != $senha) {
                        print_r('[{"status":false,"msg":"Senha incorreta!","pin":"","senha":""}]');
                        error_log('[{"status":false,"msg":"Senha incorreta!","pin":"","senha":""}]');
                    } else {
                        print_r('[{"status":false,"msg":"PIN incorreto!","pin":"","senha":""}]');
                        error_log('[{"status":false,"msg":"PIN incorreto!","pin":"","senha":""}]');
                    }
                } else {
                    if ($auth['senha'] != $senha) {
                        print_r('[{"status":false,"msg":"Senha incorreta!","pin":"","senha":""}]');
                        ...
                        error_log('[{"status":false,"msg":"Erro interno!","pin":"","senha":""}]');
                    }
                }
            }
        } else {
            print_r('[{"status":false,"msg":"Formato PIN inválido!","pin":"","senha":""}]');
            error_log('[{"status":false,"msg":"Formato PIN inválido!","pin":"","senha":""}]');
        }
        break;
...
//        } 
    ...
    case "setCompletarCadastroUsuario":
        ...
        break;
    // Finaliza pedido
    case "setFinalizarPedido":
        $pedidoDAO = new PedidoDAO();
        $entidadePedido = new EntidadePedido();
        
//        error_log("fkCliente===================> " . $_POST['fkCliente']);
//        error_log("listaIds====================> " . explode(",", trim($_POST['listaIds'])));
//        error_log("listaGruposIds==============> " . explode(",", trim($_POST['listaGruposIds'])));
//        error_log("totalCarrinho===============> " . $_POST['totalCarrinho']);
//        error_log("totalCredito================> " . $_POST['totalCredito']);
//        error_log("totalDebito=================> " . $_POST['totalDebito']);
//        error_log("totalReceberLocal===========> " . $_POST['totalReceberLocal']);

        // Carrega entidade
        $entidadePedido->setFkCliente($_POST['fkCliente']);
        $entidadePedido->setListaIds(explode(",", str_replace("[", "", str_replace("]", "", trim($_POST['listaIds'])))));
        $entidadePedido->setListaGruposIds(explode(",", str_replace("[", "", str_replace("]", "", trim($_POST['listaGruposIds'])))));
        $entidadePedido->setListaValorIndividual(explode(",", str_replace("[", "", str_replace("]", "", trim($_POST['listaValorIndividual'])))));
        $entidadePedido->setListaComentario(explode(",", str_replace("[", "", str_replace("]", "", trim($_POST['listaObs'])))));
        $entidadePedido->setTotalCarrinho($_POST['totalCarrinho']);
        $entidadePedido->setTotalCredito($_POST['totalCredito']);
        $entidadePedido->setTotalDebito($_POST['totalDebito']);
        $entidadePedido->setTotalReceberLocal($_POST['totalReceberLocal']);
        $entidadePedido->setTotal($_POST['totalCarrinho']);
        // Cadastra pedido
        $idPedido = $pedidoDAO->setCadastrarNovoPedido($entidadePedido);
        // Cadastra produtos do pedido
        $retorno = $pedidoDAO->setCadastrarProdutosNovoPedido($entidadePedido, $idPedido);
        if ($retorno) {
            print_r('[{"status":true,"msg":"Pedido recebido com sucesso!"}]');
        } else {
            print_r('[{"status":false,"msg":"Erro ao finalizar o pedido!"}]');
        }
        break;
    // Edita usuário
    case "editaUsuario":
